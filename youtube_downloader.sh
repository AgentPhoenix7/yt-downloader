...
print_help() {
  ...
  echo -e "  --audio              Force audio-only download"
  echo -e "  --video              Force audio+video download"
  echo -e "  --format FORMAT      Audio format (default: mp3)"
  echo -e "  --quality QUALITY    Video quality: best, medium, worst"
  echo -e "  --dir PATH           Download directory (default: \$HOME)"
  echo -e "  --no-clipboard       Disable clipboard auto-paste"
  echo -e "  --check-update       Check for newer script version"
  echo -e "  --update             Replace script with latest from GitHub"
  ...
  case "$1" in
    ...
    --check-update)
      ...
      if [[ "$VERSION" != "$REMOTE_VERSION" ]]; then
        echo -e "${YELLOW}Update available: $VERSION → $REMOTE_VERSION${RESET}"
    ...
    --update)
      ...
      ;;
    -h|--help) print_help ;;
    *) echo -e "${RED}Unknown option: $1${RESET}"; print_help ;;
  esac
done
